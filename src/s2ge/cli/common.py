"""Shared command-line schema and configuration resolution for S2GE.

YAML files provide defaults only. Explicit command-line arguments override
those defaults, and unknown YAML keys are rejected so misspelled experiment
settings cannot silently change a reproduction run.
"""
...
try:
    import yaml
except ImportError:  # Keep --help available before the full environment is installed.
    yaml = None
...
def csv_list(string):
    """Parse a comma-separated option while preserving an existing list."""
...
def parse_bool(value):
    """Parse common textual booleans for YAML/CLI interoperability."""
...
def load_config_defaults(config_path: str):
    """Load a YAML mapping used as parser defaults.

    Validation of key names happens in :func:`build_parser`, after the full
    public option schema has been constructed.
    """
    if not config_path:
        return {}
    if yaml is None:
        raise ImportError("Loading YAML configs requires PyYAML. Install the release requirements first.")
...
def build_parser(defaults=None):
    ...
    Arguments are ordered by responsibility.  The final block contains opt-in
    research controls; none of those controls is enabled by the paper configs.
    """
    defaults = defaults or {}
    parser = argparse.ArgumentParser(description="S2GE 2.0 trainer")
    # Run identity, data paths, and optimization.
    ...
    parser.add_argument('--use_deepspeed', '--use-deepspeed', dest='use_deepspeed', action='store_true')
    parser.add_argument(
        '--no-use-deepspeed',
        dest='use_deepspeed',
        action='store_false',
        help='Run the protocol-equivalent local PyTorch path instead of DeepSpeed.',
    )
    parser.add_argument('--deepspeed', type=str, default='')
    parser.add_argument('--local_rank', type=int, default=-1)
    # Graph encoder, sampling, and graph-to-decoder interface.
    ...
    # Optional representation-learning experiments (disabled in paper configs).
    ...
    # GRBench/Core-HopQA data selection.
    ...
    # Fixed alignment objective used by the paper protocol.
    ...
    # Experimental training controllers. These are retained for research
    # extensions and remain disabled unless explicitly enabled by a config.
    ...
    # Logging, checkpointing, validation diagnostics, and final evaluation.
    ...
    # Experimental sub-epoch validation monitor.
    ...
    unknown_config_keys = sorted(set(defaults) - valid_config_keys)
    if unknown_config_keys:
        joined = ", ".join(unknown_config_keys)
        raise ValueError(f"Unknown training config key(s): {joined}")
...
def finalize_args(args):
    """Resolve environment-dependent paths and derived run identifiers."""
    repo = repo_root()
...
def parse_args(argv=None):
    """Parse S2GE arguments with YAML defaults and strict key validation."""
