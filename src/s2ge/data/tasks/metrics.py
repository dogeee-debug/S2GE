"""Authoritative task-aware metrics for GRBench predictions."""
...
def normalize(s: str) -> str:
    """Normalize answer text for benchmark-compatible comparison."""
    s = s.lower()
...
def infer_grbench_metric_task(question=None, label=None, task_type=None):
    """Infer the metric task from explicit metadata or question templates."""
...
def compute_grbench_metrics(df, dataset=None):
    """Compute exact match, F1, hop, hallucination, and task-group metrics."""
...
def load_grbench_metrics(path, dataset=None):
    """Load prediction JSONL and compute task-aware GRBench metrics."""
...
def get_accuracy_grbench(path, args=None, dataset=None, **kwargs):
    """Return the primary benchmark score for registry compatibility."""
