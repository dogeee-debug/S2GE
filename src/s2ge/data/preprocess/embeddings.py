"""Text and lightweight feature encoders used during graph preprocessing.

Model-backed encoders honor Hugging Face offline flags and validate local model
directories before loading. ``light_text2embedding`` is deterministic and does
not require downloaded weights, making it suitable for smoke tests.
"""
...
try:
    import gensim
except ImportError:  # Optional for light/text preprocessing; required only for word2vec.
    gensim = None

try:
    from transformers import AutoModel, AutoTokenizer
except ImportError:  # Optional for light preprocessing.
    AutoModel = None
    AutoTokenizer = None
...
class _TextDataset(torch.utils.data.Dataset):
    """Tensor dataset used to batch tokenized text without extra wrappers."""
...
class SentenceTransformer(nn.Module):
    """Mean-pooled Transformer encoder compatible with local HF checkpoints."""
...
class EmbeddingBundle:
    """Loaded encoder components and their uniform text-conversion callable."""
    model: object
    tokenizer: object
    device: torch.device
...
def light_text2embedding(texts, embed_dim=1024, out_path=None, shard_size=DEFAULT_TEXT_CHUNK_SIZE):
    """Create deterministic hashed text features without external weights."""
    if embed_dim <= 0:
...
def _sbert_bundle(pretrained_repo=SBERT_REPO):
    if AutoModel is None or AutoTokenizer is None:
        raise ImportError(
            "Text embedding preprocessing requires transformers. "
            "Install the release requirements or use --grbench-feature-mode light."
        )
...
def _contriever_bundle():
    if AutoModel is None or AutoTokenizer is None:
        raise ImportError(
            "Contriever preprocessing requires transformers. Install the release requirements first."
        )
...
def _word2vec_bundle(word2vec_path):
    if gensim is None:
        raise ImportError("Word2Vec preprocessing requires gensim. Install the release requirements first.")
...
def load_embedding_bundle(model_name="sbert", word2vec_path="word2vec/GoogleNews-vectors-negative300.bin.gz"):
    """Load the requested text encoder and expose a uniform embedding API."""
