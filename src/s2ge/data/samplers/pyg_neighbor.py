"""PyG neighborhood sampling with query-role metadata preservation."""
...
def sample_graph_with_hsgs(graph, fanouts, max_seed_nodes: int, forced_seed_nodes=None):
    """Sample a local graph around prioritized seeds using HSGS fanouts.

    The returned ``n_id`` maps local nodes back to global graph IDs. Query
    source/destination/target masks are rebuilt in local coordinates so later
    ordering and alignment code never mixes the two identifier spaces.
    """
    graph_cpu = graph.cpu()
