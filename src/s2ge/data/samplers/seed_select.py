"""Deterministic seed selection for query-aware neighborhood sampling."""
...
def _degree_ranked_seeds(graph, max_seed_nodes: int):
    ...
    # Node ID is the explicit secondary key. ``torch.topk`` does not promise
    # a stable order for equal values, which can change sampled subgraphs.
    # Stable descending argsort preserves the original ascending node-ID order
    # within each equal-degree group and remains tensor-native on large graphs.
    ranked = torch.argsort(deg, descending=True, stable=True)
    return ranked[:topk].to(dtype=torch.long)


def select_seed_nodes(graph, max_seed_nodes: int, forced_seed_nodes=None):
    """Select query endpoints first, then persisted and degree-ranked seeds.

    ``forced_seed_nodes`` is used by query-conditioned runs and diagnostics
    and always has highest priority. Invalid IDs and duplicates are removed
    without reordering the remaining candidates.
    """
    num_nodes = int(graph.num_nodes)
    ordered = []
    seen = set()
