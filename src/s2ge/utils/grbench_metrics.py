"""Legacy GRBench report wrapper around the authoritative metric module.

The historical function name is retained for compatibility. Metrics that
cannot be computed from prediction JSONL alone are returned as ``NaN`` rather
than populated with misleading proxy values.
"""
import json
import re
...
def evaluate_grbench_comprehensive(prediction_path, dataset=None, args=None):
    """Report metrics derivable from predictions and mark others unavailable."""
    ...
    # Keep legacy fields visible while making unavailable values explicit.
    print("=" * 50)
    print("GRBENCH Prediction Evaluation Results")
    print("=" * 50)
    ...
    # The dictionary shape remains stable for downstream legacy scripts.
    return {
        "exact_match": em,
        "f1": f1,
        "macro_f1": macro_f1,
