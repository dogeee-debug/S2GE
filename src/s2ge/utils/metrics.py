"""Backward-compatible GRBench metric API used by dataset registry callbacks."""
...
def normalize(s: str) -> str:
    """Normalize free-form answer text before exact comparison."""
    s = s.lower()
...
def match(s1: str, s2: str) -> bool:
    """Return whether the normalized label appears in the prediction."""
...
def _hallucination_rate(pred_path, node_id_set, edge_set):
    ...
    for i in range(1, len(pred_path)):
        ...
        dst = str(pred_path[i])
        invalid_step = dst not in node_id_set or (src, dst) not in edge_set
        if invalid_step:
            invalid += 1
...
def _token_f1(pred, label):
    ...
    if len(pred_tokens) == 0 or len(label_tokens) == 0:
        return 0.0
    common = sum(1 for t in pred_tokens if t in label_tokens)
...
_ENTITY_FAMILIES = {"author_lookup", "venue_lookup", "collab_rank", "citation_rank", "graph_lookup", "other"}
_YES_WITNESS_RE = re.compile(r"^\s*yes(?:[\t, ]+(.+?))?\s*$", re.IGNORECASE)


def _infer_grbench_task_family(question=None, label=None):
    ...
    if any(
        ...
        return "path_witness"
    if any(kw in question_text for kw in ("keyword", "research interest", "area of research", "research area")):
        return "topic"
    if any(kw in question_text for kw in ("how many people at minimum", "least count of people", "to get introduced", "to be familiar with")):
        return "hop"
    if any(kw in question_text for kw in ("authors of", "who wrote the paper", "who collaborate with", "who penned both", "who within")):
        return "entity"
    if any(kw in question_text for kw in ("published", "venue did", "where did", "where has", "in which place")):
        return "entity"
    if any(kw in question_text for kw in ("affiliation", "organization is researcher", "which organization does researcher")):
        return "entity"
    if any(kw in question_text for kw in ("highest number of collaborations", "closest collaborator", "most frequently cited", "most cited", "collaborate most")):
        return "entity"
    if any(kw in question_text for kw in ("how many", "number of", "count of")):
        return "counting"
...
def _infer_grbench_task_type(question=None, label=None, task_type=None):
    inferred = _infer_grbench_task_family(question=question, label=label)
    if inferred == "topic":
        return "topic"
    if inferred == "hop":
        return "hop"
    if inferred == "counting":
        return "counting"
    if inferred == "path_witness":
        return "path_witness"
    if task_type in _ENTITY_FAMILIES:
        return "entity"
    return inferred


def _extract_hop_answer(pred_answer):
...
def _path_witness_metrics(pred_answer, label, gt_path, meta):
    ...
    return 1.0, (1.0 if pred_witness and pred_witness in valid_witnesses else 0.0)


def compute_grbench_metrics(df, dataset=None):
    """Compute exact-answer and task-aware metrics from prediction rows."""
    ...
    for pred, label, gt_path, question, task_type, meta in zip(
        ...
        label_str = str(label)
        resolved_task = _infer_grbench_task_type(question=question, label=label_str, task_type=task_type)
        if resolved_task == "topic":
            ...
            int_answer = _extract_hop_answer(pred_answer)
    ...
    for task_name, stats in task_groups.items():
        if stats["count"] == 0:
            continue
        by_task[task_name] = {"count": stats["count"], "exact_match": sum(stats["exact_match"]) / len(stats["exact_match"]) if stats["exact_match"] else 0.0, "f1": sum(stats["f1"]) / len(stats["f1"]) if stats["f1"] else 0.0}
        if task_name == "topic":
    ...
    return {"exact_match": sum(em_list) / len(em_list) if em_list else 0.0, "f1": sum(f1_list) / len(f1_list) if f1_list else 0.0, "hop_wise_accuracy": sum(hop_list) / len(hop_list) if hop_list else float("nan"), "hallucination_rate": sum(hall_list) / len(hall_list) if hall_list else float("nan"), "by_task": by_task}


def load_grbench_metrics(path, dataset=None):
    """Load prediction JSONL and compute GRBench metrics."""
    df = pd.read_json(path, lines=True)
    return compute_grbench_metrics(df, dataset=dataset)


def get_accuracy_grbench(path, args=None, dataset=None, **kwargs):
    """Dataset-registry callback returning the primary GRBench score."""
    ...
    print(f"EM: {em:.4f}")
    print(f"F1: {f1:.4f}")
    print(f"Hop-wise Accuracy: {hop_acc}")
    print(f"Hallucination Rate: {hall_rate}")
...
EVAL_FUNCS = {"grbench": get_accuracy_grbench}


def evaluate_predictions(dataset_name, prediction_path, **kwargs):
    """Evaluate predictions through the named dataset callback."""
    fn = EVAL_FUNCS[dataset_name]
    return fn(prediction_path, **kwargs)
