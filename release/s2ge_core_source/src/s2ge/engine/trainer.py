...
class Trainer:
    def __init__(self, args):
        ...
        self.base_model = build_model(args, graph_type=self.dataset.graph_type, init_prompt=getattr(self.dataset, 'prompt', None))
        self.optimizer, trainable_params = build_optimizer(self.base_model, args)
        self.runtime = DeepSpeedRuntime(args)
        self.model, self.optimizer, self.device, self.raw_model = self.runtime.initialize(self.base_model, self.optimizer, trainable_params)
        ...
        self.dynamic_align_progress_ema = None
        ...
        self.axis_progress_prev = None
        self.axis_momentum_reset_cooldown = 0
        self.axis_momentum_reset_fire_count = 0
        base_align_lambda = float(getattr(self.raw_model, "align_lambda", getattr(args, "align_lambda", 0.0)))
    ...
    def _warm_start_from_checkpoint(self):
        ...
        missing, unexpected = self.raw_model.load_state_dict(state_dict, strict=False)
        if bool(getattr(self.args, "resume_training", False)) and isinstance(checkpoint, dict):
            ...
            if "dynamic_align_progress_ema" in train_state and train_state["dynamic_align_progress_ema"] is not None:
                self.dynamic_align_progress_ema = float(train_state["dynamic_align_progress_ema"])
            if "axis_progress_prev" in train_state and train_state["axis_progress_prev"] is not None:
                self.axis_progress_prev = float(train_state["axis_progress_prev"])
            if "axis_momentum_reset_cooldown" in train_state:
                self.axis_momentum_reset_cooldown = max(0, int(train_state["axis_momentum_reset_cooldown"]))
            if "axis_momentum_reset_fire_count" in train_state:
                self.axis_momentum_reset_fire_count = max(0, int(train_state["axis_momentum_reset_fire_count"]))
    ...
    def _train_state_payload(self):
        return {
            ...
            "align_lambda": float(getattr(self.raw_model, "align_lambda", getattr(self.args, "align_lambda", 0.0))),
            "base_align_lambda": float(self._base_align_lambda),
            "dynamic_align_progress_ema": (
                float(self.dynamic_align_progress_ema) if self.dynamic_align_progress_ema is not None else None
            ),
            "axis_progress_prev": float(self.axis_progress_prev) if self.axis_progress_prev is not None else None,
            "axis_momentum_reset_cooldown": int(self.axis_momentum_reset_cooldown),
            "axis_momentum_reset_fire_count": int(self.axis_momentum_reset_fire_count),
    ...
    def _set_runtime_align_lambda(self, value):
        value = float(value)
        self.raw_model.align_lambda = value
        loss_model = getattr(self.model, "module", self.model)
        if hasattr(loss_model, "align_lambda"):
            loss_model.align_lambda = value
        if hasattr(self.base_model, "align_lambda"):
            self.base_model.align_lambda = value
    ...
    def _run_monitor_diagnostics(self, optimizer_step: int):
        ...
        predict_to_file(self.raw_model, self.monitor_loader, self.device, prediction_path)
    ...
    def _maybe_reset_axis_momentum(self, diagnostics):
        ...
        trigger_flag = 0.0
        progress_value = -1.0
        prev_value = -1.0 if self.axis_progress_prev is None else float(self.axis_progress_prev)
        delta_value = 0.0
        cooldown_value = float(self.axis_momentum_reset_cooldown)
        ...
        if is_main_process():
            if diagnostics is None:
                ...
                previous = self.axis_progress_prev
                if self.axis_momentum_reset_cooldown > 0:
                    self.axis_momentum_reset_cooldown -= 1
                if previous is not None:
                    ...
                    if self.axis_momentum_reset_cooldown <= 0 and falling and (crossed_threshold or entered_guard_band):
                        trigger_flag = 1.0
                        self.axis_momentum_reset_cooldown = self.axis_momentum_reset_cooldown_epochs
                self.axis_progress_prev = progress_value
                cooldown_value = float(self.axis_momentum_reset_cooldown)
        ...
        self.axis_progress_prev = None if progress_value < 0.0 else progress_value
        self.axis_momentum_reset_cooldown = max(0, int(round(cooldown_value)))

        if trigger_flag >= 0.5:
            zeroed_params = self._zero_optimizer_momentum()
            self.axis_momentum_reset_fire_count += 1
            if is_main_process():
                self.logger.print(
                ...
                self.logger.log(
                    {
                        ...
                        "Axis Momentum Reset Cooldown": self.axis_momentum_reset_cooldown,
                        "Axis Momentum Reset Count": self.axis_momentum_reset_fire_count,
                    }
                )
            ...
            self.logger.print(
                ...
                f"progress={progress_value:.4f}, "
                f"delta={delta_value:.4f}, "
                f"cooldown={self.axis_momentum_reset_cooldown}"
            )
            self.logger.log(
                {
                    "Axis Progress": progress_value,
    ...
    def _update_dynamic_align_lambda(self, diagnostics):
        ...
        progress_hat = None
        progress_ema = self.dynamic_align_progress_ema
        ...
        self.dynamic_align_progress_ema = None if ema_tensor.item() < 0.0 else float(ema_tensor.item())

        if is_main_process() and progress_hat is not None:
            self.logger.print(
                "Dynamic align update: "
                f"progress_hat={progress_hat:.4f}, "
                f"progress_ema={self.dynamic_align_progress_ema:.4f}, "
            ...
            self.logger.log(
                {
                    ...
                    "Dynamic Align Progress EMA": self.dynamic_align_progress_ema,
    ...
    def train_one_epoch(self, epoch):
        ...
        for step, batch in enumerate(self.train_loader):
            ...
            if is_deepspeed_enabled(self.args):
                ...
                if (step + 1) % self.args.grad_steps == 0:
            ...
            if is_main_process() and optimizer_step_completed:
                ...
                metrics = {
                    'Lr': lr,
                    'Accum Loss': accum_loss / self.args.grad_steps,
    ...
    def validate(self):
        return evaluate_loss(self.model, self.val_loader, self.device)

    def test(self):
        prediction_path = self.checkpoints.prediction_path()
        predict_to_file(self.raw_model, self.test_loader, self.device, prediction_path)
    ...
    def run_epoch_diagnostics(self, epoch):
        ...
        predict_to_file(self.raw_model, self.val_loader, self.device, prediction_path)
        diagnostics = compute_prediction_diagnostics(
            self.args.dataset,
            prediction_path,
            dataset=self.dataset,
    ...
    def fit(self):
        self.logger.init()
        if is_main_process():
            trainable_params, all_param = self.raw_model.print_trainable_params()
        ...
        for epoch in range(start_epoch, self.args.num_epochs):
            ...
            if is_main_process():
                if getattr(self.args, "save_every_epoch", False):
                    self.checkpoints.save(self.raw_model, self.optimizer, epoch, is_best=False, train_state=self._train_state_payload())
                ...
                if save_best and not self._checkpoint_failed:
                    try:
                        self.checkpoints.save(self.raw_model, self.optimizer, epoch, is_best=True, train_state=self._train_state_payload())
            ...
            if stop_training:
                break
        if getattr(self.args, "collect_hsgs_node_scores", False) and hasattr(self.raw_model, "export_hsgs_node_scores_from_tensors"):
            score_sum, score_count = self.raw_model.get_hsgs_node_score_tensors()
            ...
            if is_main_process() and score_sum is not None and score_count is not None:
                ...
                self.raw_model.export_hsgs_node_scores_from_tensors(stats_path, score_sum, score_count)
        ...
        if is_main_process():
            if getattr(self.args, "skip_test", False):
                ...
                if not self._checkpoint_failed and checkpoint_path.exists():
                    self.raw_model = self.checkpoints.load_best(self.raw_model)
                else:
